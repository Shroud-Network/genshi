...
use janus_core::proving::verifier::verify_prepare;
...
mod tests {
    ...
    use janus_core::proving::srs::SRS;
    use janus_core::proving::serialization::{proof_to_bytes, vk_to_bytes, public_inputs_to_bytes_le};
    ...
    fn test_verify_circuit_trait_proof() {
        // End-to-end: produce a proof via janus_core::api::prove::<C>(),
        ...
        impl Circuit for AddCircuit {
            ...
            const ID: &'static str = "janus-solana.test.add";

            fn num_public_inputs() -> usize { 1 }

            fn synthesize(
                builder: &mut janus_core::arithmetization::ultra_circuit_builder::UltraCircuitBuilder,
                w: &Self::Witness,
            ) -> Self::PublicInputs {
    ...
    fn test_verify_matches_native() {
        ...
        let native = janus_core::proving::verifier::verify(&proof, &vk, &pi, &srs);
