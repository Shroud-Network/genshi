...
//!
//! genshi ships a Pedersen commitment scheme on Grumpkin because Grumpkin's
//! scalar field is the BN254 base field — which makes scalar multiplication
//! cheap inside a BN254 PLONK circuit. The scheme must be both computationally
//! binding and computationally hiding, so the discrete-log relationship between
//! generator points G and H must remain unknown.
//!
...
//!
//! ## Generator Seeds
//! - G: `SHA256("genshi_pedersen_generator_G")` → try-and-increment → Grumpkin point
//! - H: `SHA256("genshi_pedersen_generator_H")` → try-and-increment → Grumpkin point
//!
//! # Commitment
//!
...
const GENERATOR_G_DOMAIN: &[u8] = b"genshi_pedersen_generator_G";

/// Domain separator for generator H.
const GENERATOR_H_DOMAIN: &[u8] = b"genshi_pedersen_generator_H";
...
///
/// Derived from: `SHA256("genshi_pedersen_generator_G")` via try-and-increment.
...
///
/// Derived from: `SHA256("genshi_pedersen_generator_H")` via try-and-increment.
