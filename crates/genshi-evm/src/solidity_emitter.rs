...
//! Solidity contract that verifies genshi PLONK-KZG proofs on-chain.
//!
//! The emitted verifier implements the same four steps as
//! [`genshi_core::proving::verifier::verify`]:
//!
...
//! layout as [`genshi_core::proving::serialization::proof_to_bytes`]
//! (big-endian uncompressed), and replays the transcript with the exact
//! same label / length-prefix encoding as
//! [`genshi_core::proving::transcript::Transcript`]. As a result, the same
...
///
/// `Default` uses a generic `genshiVerifier` contract with pragma `^0.8.24`.
/// Applications should override at least `contract_name` so multiple genshi
...
impl Default for EmitterOptions {
    fn default() -> Self {
        Self {
            contract_name: "genshiVerifier".to_string(),
            solidity_pragma: "^0.8.24".to_string(),
            title: String::new(),
            notice: "genshi PLONK-KZG proof verifier (auto-generated).".to_string(),
        }
    }
}
...
fn emit_header(out: &mut String, opts: &EmitterOptions) {
    ...
    let _ = writeln!(out, "/// @dev Generated by genshi-evm::solidity_emitter.");
    let _ = writeln!(
        out,
    ...
    let _ = writeln!(
        out,
        "///      genshi_core::proving::serialization::proof_to_bytes (BE uncompressed)."
    );
}
...
fn emit_transcript_helpers(out: &mut String) {
    ...
    // genshi_core::proving::transcript::Transcript.
    //
...
fn emit_proof_struct(out: &mut String) {
    let _ = out.write_str(r#"    // ================================================================
    // Proof struct — mirrors genshi_core::proving::prover::Proof
...
fn emit_decoder(out: &mut String) {
    ...
    // Proof byte decoder
    //
    // Byte layout (matches genshi_core::proving::serialization::proof_to_bytes):
...
fn emit_transcript_replay(out: &mut String) {
    ...
    {
        Tr memory tr = _newTranscript("genshi_plonk");
...
fn emit_verify_entrypoint(out: &mut String) {
    ...
    /// @notice Verify a genshi PLONK-KZG proof.
...
mod tests {
    ...
    fn test_generate_verifier_sol_has_all_sections() {
        ...
        assert!(sol.contains("contract genshiVerifier"));
    ...
    fn test_transcript_labels_match_verifier_rs() {
        ...
        // These are the exact labels used in genshi_core::proving::verifier.
        for label in ["genshi_plonk", "\"w\"", "\"beta\"", "\"gamma\"", "\"z\"", "\"alpha\"",
                      "\"t\"", "\"zeta\"", "\"we\"", "\"se\"", "\"ze\"", "\"zw\"",
                      "\"qe\"", "\"te\"", "\"nu\""] {
    ...
    /// into a scratch foundry project at `/tmp/genshi_fcheck/`, so the
    /// full EVM verification path can be exercised out-of-band with
    ///
    ///     forge test --match-contract genshiVerifierTest
    ///
    ...
    #[test]
    fn dump_for_forge() {
        use genshi_core::proving::serialization::{proof_to_bytes, public_inputs_to_bytes_be};
        ...
        let sol = generate_verifier_sol(&vk, &srs);
        fs::create_dir_all("/tmp/genshi_fcheck/src").unwrap();
        fs::create_dir_all("/tmp/genshi_fcheck/test").unwrap();
        fs::create_dir_all("/tmp/genshi_fcheck/testdata").unwrap();
        fs::write("/tmp/genshi_fcheck/src/genshiVerifier.sol", sol).unwrap();

        // Proof bytes as hex for `vm.parseBytes`.
        let proof_bytes = proof_to_bytes(&proof);
        let hex_proof = format!("0x{}", hex::encode(&proof_bytes));
        fs::write("/tmp/genshi_fcheck/testdata/proof.hex", hex_proof).unwrap();
        ...
        fs::write(
            "/tmp/genshi_fcheck/testdata/pi.be",
        ...
        fs::write(
            "/tmp/genshi_fcheck/foundry.toml",
            r#"[profile.default]
src = "src"
out = "out"
cache_path = "cache"
...
"#,
        ...
        fs::write(
            "/tmp/genshi_fcheck/test/genshiVerifier.t.sol",
            r#"// SPDX-License-Identifier: MIT
pragma solidity ^0.8.24;

import "forge-std/Test.sol";
import "../src/genshiVerifier.sol";

contract genshiVerifierTest is Test {
    genshiVerifier verifier;

    function setUp() public {
        verifier = new genshiVerifier();
    }
