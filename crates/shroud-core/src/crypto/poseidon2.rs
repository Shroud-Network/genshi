...
//!
//! **GUARDRAIL G5**: This is the SINGLE SOURCE OF TRUTH for all Poseidon2
//! operations in the Shroud protocol. Every component that computes a Poseidon2
//! hash (circuit gadget, SDK client-side hasher, on-chain Merkle tree updater)
//! MUST use identical parameters defined here.
//!
//! **GUARDRAIL G6**: Outputs must be bit-identical across native, WASM, and BPF
//! compilation targets.
//!
//! # Parameters
//!
...
//! - S-box: x^5
//! - State width: variable (t=3 for nullifiers, t=5 for note commitments, t=4 for Merkle)
...
pub fn poseidon2_permutation_t3(state: &mut [Fr; 3]) {
    ...
    let rc = generate_round_constants(b"shroud_poseidon2_bn254_t3", total_constants);
    let mut rc_idx = 0;
...
pub fn poseidon2_permutation_t4(state: &mut [Fr; 4]) {
    ...
    let rc = generate_round_constants(b"shroud_poseidon2_bn254_t4", total_constants);
    let mut rc_idx = 0;
...
pub fn poseidon2_permutation_t5(state: &mut [Fr; 5]) {
    ...
    let rc = generate_round_constants(b"shroud_poseidon2_bn254_t5", total_constants);
    let mut rc_idx = 0;
...
// ============================================================================
//
// These are the domain-separated hash modes used by the Shroud protocol.
...
///
/// Used for nullifier derivation: `Poseidon2(nullifier_preimage, secret, leaf_index)`
///
...
///
/// Used for note commitments: `Poseidon2(C.x, C.y, secret, nullifier_preimage, pk.x)`
///
...
mod tests {
    ...
    #[test]
    fn test_poseidon2_hash_3_nullifier_determinism() {
        // Nullifier = Poseidon2(nullifier_preimage, secret, leaf_index)
        // Same inputs must always produce same nullifier (Guardrail G2)
        let nullifier_preimage = Fr::from(42u64);
        let secret = Fr::from(100u64);
        let leaf_index = Fr::from(7u64);
        
        let n1 = poseidon2_hash_3(nullifier_preimage, secret, leaf_index);
        let n2 = poseidon2_hash_3(nullifier_preimage, secret, leaf_index);
        
        assert_eq!(n1, n2, "Nullifier derivation must be deterministic (G2)");
    }

    #[test]
    fn test_poseidon2_hash_3_leaf_sensitivity() {
        // Changing leaf_index must change the nullifier
        let nullifier_preimage = Fr::from(42u64);
        let secret = Fr::from(100u64);
        
        let n1 = poseidon2_hash_3(nullifier_preimage, secret, Fr::from(0u64));
        let n2 = poseidon2_hash_3(nullifier_preimage, secret, Fr::from(1u64));
        
        assert_ne!(n1, n2, "Different leaf indices must produce different nullifiers");
    }
    ...
    fn test_poseidon2_hash_5_deterministic() {
        ...
        assert_eq!(h1, h2, "Note commitment hash must be deterministic");
    }
