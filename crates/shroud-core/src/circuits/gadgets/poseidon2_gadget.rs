...
//! native computation.
//!
//! **GUARDRAIL G5**: Uses the SAME `generate_round_constants` and
//! parameters as the native implementation — guaranteeing identical
//! outputs for identical inputs across native, WASM, and BPF targets.
//!
//! # Design
//!
...
mod tests {
    ...
    fn test_poseidon2_hash_3_gadget_matches_native() {
        ...
        assert_eq!(builder.get_variable(result), expected, 
            "Gadget output must match native Poseidon2 hash_3 (nullifier)");
        assert!(builder.check_circuit_correctness());
    }
    ...
    fn test_poseidon2_hash_4_gadget_matches_native() {
        ...
        assert_eq!(builder.get_variable(result), expected, 
            "Gadget output must match native Poseidon2 hash_4 (Merkle)");
        assert!(builder.check_circuit_correctness());
    }
