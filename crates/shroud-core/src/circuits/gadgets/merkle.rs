//! 4-ary Poseidon2 Merkle tree gadget (depth 10, 1,048,576 leaves).
//!
//! Constrains a Merkle inclusion proof: given a leaf and a path of siblings,
//! compute the root and make it a public output.
//!
...
//! # Cost
//! 
//! 10 levels × 1 Poseidon2 hash_4 per level ≈ ~10 × 150 = ~1,500 constraints.
...
/// Merkle tree depth (10 levels for 4^10 = 1,048,576 leaves).
...
mod tests {
    ...
    fn test_merkle_single_level() {
        ...
        let root = poseidon2::poseidon2_hash_4(leaves[0], leaves[1], leaves[2], leaves[3]);
        ...
        let computed_root = merkle_inclusion_gadget(&mut builder, leaf_wire, &path);
    ...
    fn test_merkle_depth_2() {
        ...
        let computed = merkle_inclusion_gadget(&mut builder, leaf_wire, &test_path);
        assert!(builder.check_circuit_correctness());
    }
    ...
    fn test_merkle_different_positions() {
        ...
        let root0 = merkle_inclusion_gadget(&mut builder0, leaf0, &path0);
        
        let mut builder2 = UltraCircuitBuilder::new();
        let leaf2 = builder2.add_variable(leaves[2]);
        let root2 = merkle_inclusion_gadget(&mut builder2, leaf2, &path2);
